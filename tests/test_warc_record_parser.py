...
class TestWarcRecordParser(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_record.http_headers = StatusAndHeaders('200 OK', [
            ('Content-Type', 'text/html'),
        ])
        self.mock_record.content_stream().read.return_value = b'<html><body>test</body></html>'
    ...
    def test_parse_malformed_record(self):
        ...
        self.mock_record.content_stream().read.side_effect = Exception('Read error')
        record = self.parser.parse(self.mock_record)
